...
namespace Azure_Service_Bus_Tutorial
{
    class Program
    {
        ...
        //Does the work of sending a message
        ...
        //Takes a queue of messages and prepares one or more batches to send
        ...
        static async Task SendMessageBatchAsync()
        {
            ...
            await using (ServiceBusClient client = new ServiceBusClient(connectionString))
            {
                ...
                while(messages.Count > 0)
                {
                    ...
                    {
                        //dequeue the message from the .NET queue once the message is added to the match
                        messages.Dequeue();
                    }
                    else
                    {
        ...
        static async Task ReceiveMessagesAsync()
        {
            await using (ServiceBusClient client = new ServiceBusClient(connectionString))
            {
                ...
                Console.Write("Wait for a minute and then process any key to end the processing");
                Console.ReadKey();
